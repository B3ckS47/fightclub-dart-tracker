// ── PLAYER PROFILE PAGE UI MANAGER ──
// Extracted from ui-manager-dashboard.js so profiles are their own page
// (player-profile.html?id=<uuid>), deep-linkable/shareable like every other
// detail page in this app (tournament-view.html?id=, spectator.html?id=, …).
...
// Compatibility shim for database.js's deletePlayer(), which calls
// showPage('stats-page') after a successful delete — this page has no
// "stats-page" (that's back on dashboard.html), so just navigate there.
function showPage(id) {
    if (id === 'stats-page') window.location.href = 'dashboard.html';
}
...
async function loadPlayerProfile() {
    ...
    if (!player) { window.location.href = 'dashboard.html'; return; }
    ...
    // On desktop there's room to just show everything at once — Attendance
    // stays collapsed by default either way (it's a secondary panel in the
    // left column, not one of the four main detail sections).
    const isDesktop      = window.matchMedia('(min-width: 1100px)').matches;
    const defaultDisplay = isDesktop ? 'block' : 'none';
    const defaultIcon    = isDesktop ? '▼' : '▶';

    ['fold-advanced', 'fold-chart', 'fold-history'].forEach(id => {
        const body = document.getElementById(id);
        const icon = document.getElementById(id + '-icon');
        if (body) body.style.display = defaultDisplay;
        if (icon) icon.textContent = defaultIcon;
    });
    const attendBody = document.getElementById('fold-attendance');
    const attendIcon = document.getElementById('fold-attendance-icon');
    if (attendBody) attendBody.style.display = 'none';
    if (attendIcon) attendIcon.textContent = '▶';
    ...
    if (h2hSection) h2hSection.style.display = 'none'; // gated on "has H2H data" below, unrelated to open/closed
    if (h2hBody)    h2hBody.style.display    = defaultDisplay;
    if (h2hIcon)    h2hIcon.textContent       = defaultIcon;
...
<div class="h2h-matchup">
    ...
    loadPlayerProfile._history    = history;
    loadPlayerProfile._playerId   = playerId;
    loadPlayerProfile._tournWins  = tournWins;
    loadPlayerProfile._tournSec   = tournSecond;
...
function renderAdvancedStats(fromDate) {
    ...
    const allHistory  = loadPlayerProfile._history   || [];
    const tournWins   = loadPlayerProfile._tournWins  || 0;
    const tournSecond = loadPlayerProfile._tournSec   || 0;
