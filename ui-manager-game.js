...
window.addEventListener('DOMContentLoaded', () => {
    // Apply role-based UI before loading data
    const raw = sessionStorage.getItem('fc47_user');
    const user = raw ? JSON.parse(raw) : {};

    // Only admins can add or delete players
    if (user.role !== 'admin') {
...
async function openPlayerProfile(playerId) {
    ...
    // Role check after showPage so it always wins
    const currentUser = JSON.parse(sessionStorage.getItem('fc47_user') || '{}');
    const dangerZone = document.getElementById('danger-zone');
    const delBtn = document.getElementById('delete-player-btn');
    console.log('DEBUG role:', currentUser.role);
    console.log('DEBUG dangerZone:', dangerZone);
    if (currentUser.role === 'admin') {
        dangerZone.style.display = 'block';
        delBtn.onclick = () => deletePlayer(playerId, player.name);
    } else {
